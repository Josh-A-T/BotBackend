...
app.url_map.strict_slashes = False # This changes the routing so endpoints with and without slashes are treated the same
...
#bugReportID is generated by taking a unix timestamp and appending minutes and seconds to the end to prevent collisions in the future
def get_bugReportID():
...
#####
##
## /api/ pulls every end point and shows the 
##
#####
...
def api_index():
    endpoint_descriptions = {
        ...
        "DELETE /api/bugs/[string:issue_id]": "Delete a bug by issue_id",  # New endpoint
...
def delete_bug(issue_id):
    try:
        ...
        # Search for the bug with the given issue_id
        bug = bug_db.search(Bug.issue_id == issue_id)
        
        if not bug:
            return jsonify({"message": "Bug not found"}), 404
        
        # Remove the bug from the database
        bug_db.remove(Bug.issue_id == issue_id)
...
#comment out when going live and comment the above two lines    
#if __name__ == "__main__":
